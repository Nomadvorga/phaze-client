...
void main() {
    ...
    float smoothedAlpha = 1.0 - smoothstep(-1.0, thickness > 0. ? 1.0 : softness + 1.0, distance);
    ...
    if (smoothedAlpha < 0.42 && thickness > 0.0) {
        ...
        vec4 strokeColor = thickness > 0.0
                ? vec4(outlineColor.rgb, outlineColor.a * borderAlpha)
                : basicColor;

        fragColor = mix(vec4(gradient.rgb, 0.0), mix(basicColor, strokeColor, borderAlpha), smoothedAlpha);
    }
}
