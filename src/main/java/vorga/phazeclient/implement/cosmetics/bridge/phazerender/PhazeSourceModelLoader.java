package vorga.phazeclient.implement.cosmetics.bridge.phazerender;
...
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.PhazeSourceModel.AnimationData;
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.PhazeSourceModel.BoneAnimationData;
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.PhazeSourceModel.Channel;
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.geo.GeoModel;
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.geo.GeoModelParser;
...
public final class PhazeSourceModelLoader {
    private static final Map<Integer, PhazeSourceModel> CACHE = new ConcurrentHashMap<>();

    static PhazeSourceModel get(CosmeticEntry entry) {
    ...
    private static PhazeSourceModel load(CosmeticEntry entry) {
        try (InputStream in = PhazeSourceModelLoader.class.getClassLoader().getResourceAsStream(entry.modelResource())) {
            ...
            System.err.println("[PhazeCosmetics] Phaze renderer failed to load " + entry.name() + ": " + e);
            return null;
        }
    }
    ...
    private PhazeSourceModelLoader() {}
}
