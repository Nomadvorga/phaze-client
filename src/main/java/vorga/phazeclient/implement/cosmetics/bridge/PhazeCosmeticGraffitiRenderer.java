...
public final class PhazeCosmeticGraffitiRenderer {
    ...
    public static synchronized void register() {
        ...
        WorldRenderEvents.AFTER_ENTITIES.register(ctx -> {
            ...
            int rendered = 0;
            for (CosmeticsSyncService.PhazeGraffiti graffiti
                    : CosmeticsSyncService.getInstance().phazeGraffiti()) {
                if (!serverKey.equals(graffiti.serverKey())
                        || !dimension.equals(graffiti.dimension())) continue;
                BlockPos pos = graffiti.pos();
                if (pos.toCenterPos().squaredDistanceTo(client.player.getEntityPos()) > MAX_DISTANCE_SQ) continue;
                ...
                render(entry, pos, graffiti.face(), ctx.matrices(), ctx.consumers(), camera);
    ...
    private static void render(CosmeticEntry entry, BlockPos pos, Direction face,
        ...
        VertexConsumer consumer = providers.getBuffer(RenderLayers.entityTranslucent(texture));
    ...
    private PhazeCosmeticGraffitiRenderer() { }
}
