...
import vorga.phazeclient.implement.cosmetics.bridge.phazerender.PhazeGeoRenderer;
...
public final class PhazeCosmeticRenderer {
    ...
    public static void renderBody(MatrixStack matrices, VertexConsumerProvider providers,
        ...
        if (player == null || !CosmeticsRenderer.shouldRenderFor(state) || state.spectator) return;
        float time = time();
    ...
    public static void renderHead(MatrixStack matrices, VertexConsumerProvider providers,
        ...
        PhazeGeoRenderer.render(entry, matrices, providers, light,
    ...
    private static CosmeticEntry entry(AbstractClientPlayerEntity player, CosmeticCategory category) {
        return CosmeticsSyncService.getInstance().phazeEntryFor(player.getUuid(), category);
    }
