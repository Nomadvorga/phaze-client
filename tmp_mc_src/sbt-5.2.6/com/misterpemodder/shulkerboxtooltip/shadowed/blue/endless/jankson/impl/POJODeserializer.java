...
import com.misterpemodder.shulkerboxtooltip.shadowed.blue.endless.jankson.annotation.Deserializer;
         ...
         if (elem == JsonNull.INSTANCE) {
            ...
            if (!isCollections(fieldClass)) {
               ...
               if (Map.class.isAssignableFrom(fieldClass)) {
                  ...
                  unpackMap((Map)field.get(parent), parameters[0], parameters[1], elem, marshaller);
                  return true;
               } else if (Collection.class.isAssignableFrom(fieldClass)) {
                  Type elementType = ((ParameterizedType)field.getGenericType()).getActualTypeArguments()[0];
                  unpackCollection((Collection)field.get(parent), elementType, elem, marshaller);
                  return true;
               } else {
                  return false;
               }
      ...
      if (!m.getReturnType().equals(targetClass)) {
         ...
         if (params.length == 1) {
            ...
            return params[1].getClass().equals(Marshaller.class) ? (o, marshaller) -> {
               try {
                  return m.invoke((Object)null, o, marshaller);
               } catch (IllegalArgumentException | InvocationTargetException | IllegalAccessException ex) {
                  throw new DeserializationException(ex);
               }
            } : null;
